// fsb_utils!(160, 5 << 18, 80, 640, 653, 1120)

macro_rules! fsb_utils {
    (
        $outputsize:expr, $n:expr, $w:expr, $r:expr,
        $p:expr, $s:expr
    ) => {
        const N: usize = $n;
        // number of indexes
        ...
        const HASH_OUTPUT_SIZE: usize = $outputsize / 8; // size (in bits) divided by 8 (to bytes)
