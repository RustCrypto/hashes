...
//!
//! ```
//! use blake2::blake2b::{blake2b, State, many::update_many};
//!
//! let mut states = [
//!     State::new(),
//!     State::new(),
...
#[inline(always)]
fn evict_finished(vec: &mut JobsVec<'_, '_>, num_jobs: usize) {
...
///
/// ```
/// use blake2::blake2b::{Params, many::{HashManyJob, hash_many}};
///
/// let inputs = [
///     &b"foo"[..],
///     &b"bar"[..],
...
mod test {
    use super::*;
    use crate::blake2b::{backend, paint_test_input, BLOCKBYTES};
    use arrayvec::ArrayVec;
    ...
    fn test_hash_many() {
        ...
        for start_offset in 0..LEN {
            ...
            let mut jobs: ArrayVec<[HashManyJob<'_>; LEN]> = ArrayVec::new();
            for i in 0..LEN {
