...
use digest::{
    ...
    typenum::{U32, U56, Unsigned},
    crypto_common::hazmat::{SerializedState, SerializableState, DeserializeStateError},
};
...
impl SerializableState for BeltHashCore {
    ...
    fn serialize(&self) -> SerializedState<Self> {
        ...
        let (r_dst, tail) = dst.split_at_mut(8);
        let (s_dst, h_dst) = tail.split_at_mut(16);

        r_dst[20..].copy_from_slice(&self.r.to_le_bytes());

        for (val, chunk) in self.s.iter().zip(s_dst.chunks_exact_mut(4)) {
            chunk.copy_from_slice(&val.to_le_bytes());
        }

        for (val, chunk) in self.h.iter().zip(h_dst.chunks_exact_mut(4)) {
            chunk.copy_from_slice(&val.to_le_bytes());
        }
    ...
    fn deserialize(
        ...
        let (r_src, tail) = serialized_state.split_at(8);
        let (s_src, h_src) = tail.split_at(16);

        let r = u128::from_le_bytes(r_src.try_into().unwrap());

        let mut s = [0u32; 4];
        for (val, chunk) in s.iter_mut().zip(s_src.chunks_exact(4)) {
            *val = u32::from_le_bytes(chunk.try_into().unwrap());
        }

        let mut h = [0u32; 8];
        for (val, chunk) in h.iter_mut().zip(h_src.chunks_exact(4)) {
            *val = u32::from_le_bytes(chunk.try_into().unwrap());
        }
